"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import { useSession } from "@/lib/auth-client";
import { AppHeader } from "@/components/app-header";
import { Footer } from "@/components/footer";
import { useToast } from "@/components/toast";
import { formatTimeAgo } from "@/lib/time";

type Feedback = {
  id: string;
  title: string;
  description: string | null;
  category: string;
  status: string;
  votes: number;
  created_at: string;
  author_name: string;
  author_image: string | null;
  user_voted: boolean;
};

const statusColors: Record<string, string> = {
  open: "text-code-green border-code-green/30",
  "in-progress": "text-code-blue border-code-blue/30",
  planned: "text-purple-400 border-purple-400/30",
  completed: "text-yellow-400 border-yellow-400/30",
  declined: "text-red-400 border-red-400/30",
};

const categoryColors: Record<string, string> = {
  feature: "text-code-blue",
  bug: "text-red-400",
  improvement: "text-code-green",
  general: "text-medium-gray",
};

export default function FeedbackPage() {
  const { data: session } = useSession();
  const { toast } = useToast();
  const [feedback, setFeedback] = useState<Feedback[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [category, setCategory] = useState("feature");
  const [submitting, setSubmitting] = useState(false);
  const [filter, setFilter] = useState<"all" | "feature" | "bug" | "improvement">("all");

  useEffect(() => {
    if (!session) return;
    fetch("/api/feedback")
      .then((r) => (r.ok ? r.json() : []))
      .then(setFeedback)
      .finally(() => setLoading(false));
  }, [session]);

  async function submit() {
    if (!title.trim()) return;
    setSubmitting(true);
    const res = await fetch("/api/feedback", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: title.trim(), description: description || undefined, category }),
    });
    if (res.ok) {
      const item = await res.json();
      setFeedback((prev) => [{ ...item, author_name: session!.user.name, user_voted: false }, ...prev]);
      setTitle("");
      setDescription("");
      setShowForm(false);
      toast("Feedback submitted!", "success");
    }
    setSubmitting(false);
  }

  async function toggleVote(feedbackId: string) {
    const res = await fetch(`/api/feedback/${feedbackId}/vote`, { method: "POST" });
    if (res.ok) {
      const data = await res.json();
      setFeedback((prev) =>
        prev.map((f) =>
          f.id === feedbackId
            ? { ...f, user_voted: data.voted, votes: f.votes + (data.voted ? 1 : -1) }
            : f
        )
      );
    }
  }

  const filtered = filter === "all" ? feedback : feedback.filter((f) => f.category === filter);

  if (!session) {
    return (
      <div className="min-h-screen">
        <AppHeader breadcrumb={{ label: "Feedback", href: "/feedback" }} />
        <main className="mx-auto max-w-3xl px-4 pt-24 pb-16 text-center">
          <p className="text-medium-gray">
            <Link href="/login" className="text-code-blue hover:text-code-green">Log in</Link> to submit feedback.
          </p>
        </main>
        <Footer />
      </div>
    );
  }

  return (
    <div className="min-h-screen">
      <AppHeader breadcrumb={{ label: "Feedback", href: "/feedback" }} />

      <main className="mx-auto max-w-3xl px-4 pt-24 pb-16">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold">Feedback</h1>
            <p className="text-sm text-medium-gray">Share ideas and vote on what matters most</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="px-4 py-2 text-xs font-semibold border border-code-green bg-code-green text-black hover:bg-transparent hover:text-code-green transition-colors"
          >
            {showForm ? "Cancel" : "Submit Feedback"}
          </button>
        </div>

        {showForm && (
          <div className="border border-medium-gray/20 p-4 mb-6 space-y-3">
            <h2 className="text-xs font-semibold uppercase tracking-widest text-code-green">
              {"// new feedback"}
            </h2>
            <input
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="Short title for your feedback"
              className="w-full border border-medium-gray/30 bg-black px-3 py-2 text-sm text-white placeholder:text-medium-gray"
            />
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Describe in detail (optional)"
              rows={3}
              className="w-full border border-medium-gray/30 bg-black px-3 py-2 text-sm text-white placeholder:text-medium-gray resize-none"
            />
            <div className="flex items-center gap-3">
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="border border-medium-gray/30 bg-black px-2 py-1.5 text-xs text-white"
              >
                <option value="feature">Feature Request</option>
                <option value="bug">Bug Report</option>
                <option value="improvement">Improvement</option>
                <option value="general">General</option>
              </select>
              <button
                onClick={submit}
                disabled={submitting || !title.trim()}
                className="px-4 py-1.5 text-xs font-semibold border border-code-green text-code-green hover:bg-code-green hover:text-black transition-colors disabled:opacity-50"
              >
                {submitting ? "Submitting..." : "Submit"}
              </button>
            </div>
          </div>
        )}

        {/* Filters */}
        <div className="flex gap-2 mb-4">
          {(["all", "feature", "bug", "improvement"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1 text-xs transition-colors ${
                filter === f
                  ? "bg-code-green/10 text-code-green border border-code-green/30"
                  : "text-medium-gray hover:text-white border border-medium-gray/20"
              }`}
            >
              {f === "all" ? "All" : f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>

        {/* Feedback list */}
        {loading ? (
          <p className="text-sm text-medium-gray">Loading...</p>
        ) : filtered.length === 0 ? (
          <div className="border border-medium-gray/20 p-8 text-center">
            <p className="text-sm text-medium-gray">No feedback yet. Be the first to share!</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map((f) => (
              <div key={f.id} className="flex gap-3 border border-medium-gray/20 p-4">
                {/* Vote button */}
                <button
                  onClick={() => toggleVote(f.id)}
                  className={`flex flex-col items-center justify-center shrink-0 w-12 border transition-colors ${
                    f.user_voted
                      ? "border-code-green/30 bg-code-green/10 text-code-green"
                      : "border-medium-gray/20 text-medium-gray hover:border-code-green/30 hover:text-code-green"
                  }`}
                >
                  <span className="text-xs">^</span>
                  <span className="text-sm font-bold">{f.votes}</span>
                </button>

                {/* Content */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs ${categoryColors[f.category] || "text-medium-gray"}`}>
                      [{f.category}]
                    </span>
                    <span className={`text-xs px-1.5 py-0.5 border ${statusColors[f.status] || "text-medium-gray border-medium-gray/30"}`}>
                      {f.status}
                    </span>
                  </div>
                  <h3 className="text-sm font-semibold text-light-gray">{f.title}</h3>
                  {f.description && (
                    <p className="text-xs text-medium-gray mt-1 line-clamp-2">{f.description}</p>
                  )}
                  <div className="flex items-center gap-2 mt-1.5 text-xs text-medium-gray">
                    <span>{f.author_name}</span>
                    <span>&middot;</span>
                    <span>{formatTimeAgo(f.created_at)}</span>
                  </div>
                </div>
              </div>
            ))}
          </div>
        )}
      </main>
      <Footer />
    </div>
  );
}
